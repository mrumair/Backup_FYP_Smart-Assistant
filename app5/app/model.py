...
class post:
  ...
  def RetrieveAgenda(self):
    listStoreAg = list()
    #queryNode = 'MATCH (n:UserMeeting) WHERE (n)--() RETURN n'
    queryNode = 'start n=node(*) match (n:UserMeeting)-[:meet]-() return distinct n'
  ...
  def CreateNode_Ag(subjects , propertys, detail, noofmems, supervisor):
    title = subjects
    nodeType = propertys
    tdetail = detail
    tnoofmems = noofmems
    tsupervisor = supervisor
    query= 'MERGE (n:UserMeeting {name:{sname} , type: {type}, detail: {sdetail}, noOfmems: {snoofmems}, supervisor:{ssupervisor}}) RETURN n.name' 
    post=graph.run(query,sname=title , type = nodeType , sdetail=tdetail, snoofmems=tnoofmems,ssupervisor=tsupervisor ) 
    print("Node Created!!") 
    print(post.data())
  ...
  def CreateRelation_Ag(subjct , objct , relation):
    ...
    obj = objct
    rel_type = relation
    query =  'MERGE (u1:UserMeeting { name: {subs} }) MERGE (u2:UserMeeting { name:{objs} }) MERGE (u1)-[:has {name: {rel}}]-(u2)'
